...
function buildSections(patch: string, prize: string) {
  return [
    {
      title: "Formato",
      rules: [
        "Eliminación simple a Bo1. La final se juega a Bo3.",
        `Se juega en el parche ${patch} — Grieta del Invocador, modo Torneo (draft con bans).`,
...
export default async function ReglasPage() {
  ...
  const sections = buildSections(t?.patch ?? "—", t?.prize ?? "por anunciar");
